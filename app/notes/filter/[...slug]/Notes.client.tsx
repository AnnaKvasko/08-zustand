...
export default function NotesClient({
  ...
  const derivedTag = useMemo<NoteTag | "all">(() => {
    const parts = pathname.split("/").filter(Boolean);
    // parts: ["notes", "filter", "Todo"]

    if (parts[0] === "notes" && parts[1] === "filter" && parts[2]) {
      return parts[2] as NoteTag | "all";
    }
  ...
  const tagForQuery = useMemo<NoteTag | undefined>(() => {
    return derivedTag !== "all" ? (derivedTag as NoteTag) : undefined;
  }, [derivedTag]);

  const basePath = useMemo(() => {
    if (derivedTag && derivedTag !== "all") {
      return `/notes/filter/${encodeURIComponent(derivedTag)}`;
    }
    if (derivedTag === "all") return `/notes/filter/all`;
    return `/notes`;
  }, [derivedTag]);
  ...
  const queryKey = useMemo(
    () =>
      [
        "notes",
        { page, search: debouncedSearch, perPage, tag: tagForQuery ?? "all" },
      ] as const,
    [page, debouncedSearch, perPage, tagForQuery]
  );

  const baseParams = { page, perPage, search: debouncedSearch || undefined };
  const paramsFetch = tagForQuery
    ? { ...baseParams, tag: tagForQuery }
    : baseParams;
    ...
    >
      <header className={css.toolbar}>
        ...
        {pages > 1 && (
          <div className={css.centerBlock}>
            <Pagination
              key={`top-${pages}-${derivedTag ?? "all"}-${debouncedSearch}`}
              pageCount={pages}
              currentPage={page}
              onPageChange={onPageChange}
            />
      ...
      {pages > 1 && (
        <Pagination
          key={`bottom-${pages}-${derivedTag ?? "all"}-${debouncedSearch}`}
          pageCount={pages}
          currentPage={page}
          onPageChange={onPageChange}
        />
